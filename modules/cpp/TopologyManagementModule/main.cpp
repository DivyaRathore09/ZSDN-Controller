...
int main(int argc, char* argv[]) {
    ...
    if(zsdn::StartupHelper::paramsOkay(argc,argv)) {
        ...
        zmf::logging::ZmfLogging::initializeLogging("TopologyManagementModule", argv[1]); 
        // Start the module
        returnCode = zsdn::StartupHelper::startInConsole(new TopologyManagementModule(instanceId), argv[1],startOptions);
    } else {
        returnCode = 1;
    }
    return returnCode;
